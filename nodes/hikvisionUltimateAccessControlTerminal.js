...
module.exports = function (RED) {
	function hikvisionUltimateAccessControlTerminal(config) {
		...
		const getEventDescription = (payload) => {
			if (!payload || typeof payload !== "object") return "Unknown event";
			if (payload.eventDescription) return payload.eventDescription;
			const major = payload.hasOwnProperty("major") ? payload.major : "?";
			const minor = payload.hasOwnProperty("minor") ? payload.minor : "?";
		...
		node.sendPayload = (_msg) => {
			...
			if (_msg.hasOwnProperty("errorDescription")) {
				...
				// Normalize event description so missing properties from some devices do not break the node
				const payload = _msg.payload || {};
				const eventDescription = getEventDescription(payload);
				...
				if (node.filtermajorevent > 0) {
					if (Number(_msg.payload.major) === node.filtermajorevent) {
						if (node.filterminorevent > 0) {
							if (Number(_msg.payload.minor) === node.filterminorevent) {
								node.send([_msg, null]);
								setTimeout(() => {
									node.setNodeStatus({ fill: "green", shape: "dot", text: "Event " + eventDescription });
								}, 1000);
								logDebug(`Forwarded filtered event major ${_msg.payload.major} minor ${_msg.payload.minor}`);
							} else {
								try {
									setTimeout(() => {
										node.setNodeStatus({ fill: "grey", shape: "dot", text: "Filtered " + eventDescription });
									}, 500);
								} catch (error) { }
					...
					logDebug(`Forwarded event ${eventDescription || ""} without filters`);
				}
